...
- **⚡️ Autonomous curriculum flow is ON (Alex enabled 2026-06-28):** CC self-authors + self-merges curriculum units on full green (lint+validate+tests+smoke+build); native review batched; Claude spot-review on request. Shipped under it: `kanji` type (#44), **Unit 11 first kanji** + **Unit 12 colors/weather** (#45). **Next curriculum:** more kanji (days-of-week 曜日, time), particles/grammar; then A2 — plus batch the U11 kanji to a native reviewer.
- **Decisions / to-dos for Alex:** **accounts in production** — add `VITE_SUPABASE_*` env to Vercel + redeploy + add the prod URL to Supabase redirect allow-list (Apple sign-in later, needs Apple Dev account). Unit 4–10 native/Claude review pass still open.
...
- [ ] More kanji (days-of-week 曜日, time 時分, common N5 set) + the U11 batch → **native-speaker review** (per the autonomous workflow). (CC/native)
